...
module fruit
  ...
  character (len = *), parameter :: default_xml_filename = "result.xml"
  character (len = 256) :: xml_filename = default_xml_filename
...
!$omp end critical (FRUIT_OMP_DEALLOCATE_MESSAGE_ARRAY)
  ...
  subroutine init_fruit_xml_(xml_file_name, rank)
    character(len=*), optional, intent(in) :: xml_file_name
    integer, optional, intent(in) :: rank
    logical :: rank_zero_or_single

    if (present(xml_file_name)) then
      xml_filename = trim(xml_file_name)
    endif
